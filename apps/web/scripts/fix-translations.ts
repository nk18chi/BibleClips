...
async function fixTranslations() {
  ...
  for (const [seq, translation] of Object.entries(matthewFixes)) {
    const word = mWords?.find((w) => w.sequence === parseInt(seq, 10));
    if (word) {
      console.log(`  ${seq}: "${word.word}" -> "${translation.slice(0, 30)}..."`);
      await supabase.from("clip_subtitles").update({ word_ja: translation }).eq("id", word.id);
    }
  }
  ...
  for (const [seq, translation] of Object.entries(philFixes)) {
    const word = pWords?.find((w) => w.sequence === parseInt(seq, 10));
    if (word) {
      console.log(`  ${seq}: "${word.word}" -> "${translation.slice(0, 30)}..."`);
      await supabase.from("clip_subtitles").update({ word_ja: translation }).eq("id", word.id);
    }
