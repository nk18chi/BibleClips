...
async function seedSampleClip() {
  ...
  // First, we need a user to be the submitter
  // For now, let's create a system user or use service role to bypass RLS

  // Check if categories exist, if not seed them
  const { data: existingCategories } = await supabase.from("categories").select("id, slug").limit(1);
  ...
  // Insert the sample clip (without submitted_by for now - we'll need to handle this)
  // Since we can't insert without a user due to foreign key, let's use a raw insert
  const clipId = crypto.randomUUID();

  // Insert clip directly - note: this might fail due to foreign key constraints
  // We need to either create a system user or modify the schema
  ...
  if (clipError) {
    ...
    console.log("Note: This might fail due to submitted_by foreign key constraint.");
    console.log("You may need to run migrations first and create a user.");
    return;
  }
